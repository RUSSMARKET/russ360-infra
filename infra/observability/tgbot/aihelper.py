...
Формат вывода — это Telegram-чат, не Markdown-документ. Правила:
- НЕ рисуй таблицы (в Telegram они разъезжаются). Вместо таблицы — короткие строки вида «сервис: 120 req, p95 0.3s».
- НЕ используй заголовки (#), символы ** и __ для жирного, ~~ для зачёркивания, знаки тильды.
- Не используй длинное тире (—) и стрелки-глифы (→, ←). Пиши обычными дефисами.
- Списки — короткими строками, каждый пункт с новой строки, максимум один уровень.
- Пиши живым текстом, как инженер коллеге в чат, а не как отчёт."""
