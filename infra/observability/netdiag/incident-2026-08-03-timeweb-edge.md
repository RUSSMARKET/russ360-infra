...
Контрольный тест после полного SSO-проксирования в 17:00–17:02 МСК успешно
прошёл с подтверждённого Yota CGNAT `94.25.*`: authorize/callback, token,
`/api/auth/me`, userinfo, продукты и рабочие API вернули `200`, рабочий стол
открылся без ручного повторного ввода данных. Это первый полностью
функциональный проход приложения через второй публичный адрес на том же
проблемном телефоне и мобильной сети.

При выходе обнаружился отдельный canary-only loop: `/oidc/logout` завершался
с `302` на `/auth`, после чего SPA автоматически начинала вход снова и
возвращала пользователя на рабочий стол. Logout-return на canary теперь ведёт
на статический no-store экран «Вы вышли из профиля» с только ручной кнопкой
входа. Два best-effort запроса `/oauth/revoke` от публичного SPA-клиента всё
ещё получают `401`; это отдельная задача OAuth hardening, не причина сетевого
инцидента и не препятствие очистке локальной и SSO browser-сессии.
