...
- **rusaifin dev — ✅ DONE (Sentry + /metrics + JSON-логи; scrape отложен).** Разобрался: 11 «ahead»-коммитов = чистые merge-бабблы (`diff origin/dev...HEAD` пуст, без уникального контента); :8080 = **Apache** (native PHP), Hestia-nginx проксирует. `git pull origin dev` (merge, как у них и заведено) принёс obs-код **без конфликтов**. `composer install` (composer:2 контейнер, uid fintech) → sentry+promphp. `.env`: DSN id3, env=dev, PROMETHEUS_STORAGE=apcu, **LOG_CHANNEL=json** (native). `package:discover`+`config:clear` (php8.3). Верифицировано: `/metrics`=200 с `russ360_http_requests_total`, Sentry-событие в GlitchTip project 3, JSON в laravel.log. Reload не понадобился (opcache validate_timestamps).
  - **Scrape rusaifin отложен** (native, не на app-network): Prometheus его не скрейпит. Чтобы добавить — нужен job на `host.docker.internal:8080` с Host-заголовком `dev.server.rusaifin.ru` ИЛИ публичный `dev.server.rusaifin.ru/metrics` (тогда обязателен nginx allowlist на Hestia-vhost — сейчас /metrics публичен на dev). /metrics-endpoint работает, ждёт scrape-решения.
  - ⚠️ **Security:** remote `findatabase` содержит GitHub PAT открытым текстом в `.git/config` (`https://Russ:ghp_...@github.com/...`). Не сохранял токен. Рекомендация: ротировать PAT + перейти на credential-helper/SSH-deploy-key.
