...
- **blackbox-exporter** — внешняя проверка доступности (TLS-срок, редиректы, реальный ответ) для Grafana/SSO/обоих фронтов, SMTP TLS, TCP-маршрута к двум базам 1С и наличия netdiag beacon в production HTML; ловит то, что внутренний scrape не видит.
