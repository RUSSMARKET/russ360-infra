...
- Остались hardening-задачи, не связанные с инцидентом: публичные Apache
  `8080/8443`, MySQL listener `3306`, конфликтующий legacy mail vhost и три
  legacy vhost с просроченными сертификатами (два уже не имеют DNS, один
  suspended). Их нельзя смешивать с исправлением пользовательского маршрута.
